...
<body>
    ...
    <script>
        ...
        function initiateDownload() {
            ...
            window.location.href = `/download?url=${encodeURIComponent(url)}&quality=${document.getElementById('quality').value}`;
        }
    </script>
</body>
</html>
...
@app.route('/')
def home():
    return render_template_string(HTML_CODE, profile_pic=PHOTO_URL, fb=MY_FACEBOOK, tk=MY_TIKTOK, ig=MY_INSTAGRAM)
...
def download():
    ...
    try:
        ydl_opts = {'format': 'best'} if quality == 'best' else {'format': 'bestaudio/best', 'postprocessors': [{'key': 'FFmpegExtractAudio','preferredcodec': 'mp3','preferredquality': '192'}]}
